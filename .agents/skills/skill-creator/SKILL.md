...
name: skill-creator
description: >-
  Guide for creating effective skills. Use when a user requests a new or updated
  skill, or when you (Agent) determine a skill should be created to extend Agent
...
- **Self-contained runtime surfaces**: If a skill depends on reusable runtime scaffolds (for example review/planning templates, checklists, form skeletons, or note scaffolds), keep portable copies inside the skill package—typically under `assets/`. Do not make an external system's local template folder the only place those surfaces exist. Local adapters/mirrors are acceptable when truly needed, but they should not be the sole source.
- **Local syntax is okay when useful**: Portable assets may still use system-native markdown/features such as `[[wikilinks]]` when that supports the workflow. Portability means the skill carries its own needed surfaces/resources, not that every asset must avoid all local syntax.
